...
func main() { 
    ...
    /* 定义一个16位的整型数，第二行进行2个变量运算。 由于变量的类型不一样，因此需要进行强制类型转换。 */
    var var_name_x int16
    var_name_x = 11 + int16(var_name)
	
	/* 上面介绍的都是数字型的数据类型，除了上面的类型外，还有int32，int64，uint8，uint16，uint32，uint64等类型，这些表示的是整型数。而以uint头的表示无符号整型数。 */

    fmt.Println(var_name_z)
	
	/* 除了整型数外，还有浮点型数，浮点型数就是我们日常说的小数 */
	...
	/* 可以同时声明多个变量，如下是定义3个都为int32类型的变量，并进行初始化，初始化并非强制要求的 */
